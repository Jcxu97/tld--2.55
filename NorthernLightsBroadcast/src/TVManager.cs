...
public class TVManager : MonoBehaviour
{
	...
	public AudioSource videoAudioSource; // points to playerAudio._audioSource for TVUI compat

	public WasapiAudioPlayer wasapiPlayer = new WasapiAudioPlayer();
	...
	public void Awake()
	{
		...
		videoAudioSource = playerAudio._audioSource; // alias for TVUI volume/mute control
	...
	public void SwitchState(TVState newState)
	{
		...
		case TVState.Playing:
			...
			staticAudio.Stop();
			redbutton.Glow(enabled: true);
			double startTime = FileStuff.GetFrameValueFromFile(ui.currentClip);
			videoPlayer.time = startTime;
			videoPlayer.Play();
			wasapiPlayer.Open(ui.currentClip);
			wasapiPlayer.Volume = ui.audioSlider.value;
			if (startTime > 1.0)
				wasapiPlayer.Seek(startTime);
			wasapiPlayer.Play();
			((Behaviour)ambilight).enabled = false;
