...
public class TVUI : MonoBehaviour
{
	...
	private float lastProgressInteraction;

	private bool isSyncingProgress;
	...
	public void Awake()
	{
		...
		manager.videoAudioSource.volume = audioSlider.value;
		...
		// 扩大 handle 可点击区域但不改变视觉: 在 handle 上叠一个透明大碰撞层
		if (progressBar.handleRect != null)
		{
			var hitZone = new GameObject("HitZone");
			hitZone.transform.SetParent(progressBar.handleRect, false);
			var rt = hitZone.AddComponent<RectTransform>();
			rt.anchorMin = new Vector2(0.5f, 0.5f);
			rt.anchorMax = new Vector2(0.5f, 0.5f);
			rt.sizeDelta = new Vector2(20f, 12f);
			var img = hitZone.AddComponent<Image>();
	...
	public void PopulateFiles()
	{
		...
		for (int j = 0; j < 8; j++)
		{
			if (j + currentPage * 8 < folderContents.Count)
			{
				...
				{
					((Graphic)listText[j]).color = (Color)folderColor;
					listSprites[j].sprite = NorthernLightsBroadcastMain.folderIconSprite;
				}
				else
				{
					((Graphic)listText[j]).color = (Color)videoColor;
	...
	[HideFromIl2Cpp]
	public void SyncProgressBarOnce()
	{
		if (manager.currentState == TVManager.TVState.Playing || manager.currentState == TVManager.TVState.Paused)
		{
			if (manager.videoPlayer.frameCount > 0)
			{
				isSyncingProgress = true;
				progressBar.maxValue = (float)manager.videoPlayer.frameCount;
				progressBar.SetValueWithoutNotify((float)manager.videoPlayer.frame);
	...
	public void Mute()
	{
		manager.videoAudioSource.mute = !manager.videoAudioSource.mute;
		manager.staticAudio._audioSource.mute = !manager.staticAudio._audioSource.mute;
		manager.wasapiPlayer.Volume = manager.videoAudioSource.mute ? 0f : audioSlider.value;
	}
	...
	public void VolumeSlider()
	{
		...
		manager.videoAudioSource.volume = vol;
		manager.staticAudio._audioSource.volume = vol;
		manager.wasapiPlayer.Volume = vol;
		SaveLoad.SetVolume(manager.thisGuid, vol);
	}
	...
	public void ProgressBar()
	{
		...
		if (manager.currentState != TVManager.TVState.Playing &&
			manager.currentState != TVManager.TVState.Paused) return;
		if (manager.videoPlayer.frameCount <= 0) return;
		lastProgressInteraction = Time.unscaledTime;
		progressBar.maxValue = (float)manager.videoPlayer.frameCount;
		long targetFrame = (long)progressBar.value;
		if (targetFrame >= 0 && targetFrame < (long)manager.videoPlayer.frameCount)
		{
			manager.videoPlayer.frame = targetFrame;
			double seekTime = targetFrame / (double)manager.videoPlayer.frameRate;
			manager.wasapiPlayer.Seek(seekTime);
		}
	}

	public void Update()
	{
		if (OSDOpen && manager.currentState == TVManager.TVState.Playing)
		{
			manager.UpdateTimeText();
		}
	}

	public void OnApplicationFocus(bool hasFocus)
	{
		if (hasFocus && manager != null && manager.currentState == TVManager.TVState.Playing)
		{
			SyncProgressBarOnce();
		}
	}
