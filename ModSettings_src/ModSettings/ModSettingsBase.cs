...
public abstract class ModSettingsBase
{
	...
	private class Visibility
	{
		...
		private void ChangeVisibility(bool visible)
		{
			...
			foreach (OnVisibilityChange visibilityListener in visibilityListeners)
			{
	...
	protected ModSettingsBase()
	{
		...
		menuVisibility.SetVisible(visible: false);
		menuVisibility.AddVisibilityListener(delegate(bool visible)
		{
			if (visible)
			{
			...
			else
			{
				...
				{
					fieldInfo2.SetValue(this, confirmedValues[fieldInfo2]);
				}
			}
		});
		...
		foreach (FieldInfo key in array)
		{
			fieldVisibilities.Add(key, new Visibility(visibility));
		}
	...
	{
		FieldInfo fieldForName = GetFieldForName(fieldName);
		return IsFieldVisible(fieldForName);
	}
	...
	{
		FieldInfo fieldForName = GetFieldForName(fieldName);
		SetFieldVisible(fieldForName, visible);
	}
	...
	internal void SetFieldValue(FieldInfo field, object newValue)
	{
		...
		if (value != newValue)
		{
			...
			CallOnChange(field, value, newValue);
		}
	}
	...
	internal void CallOnConfirm()
	{
		...
		{
			MelonLogger.Error("Exception in OnConfirm handler\n" + ex.ToString());
		}
		FieldInfo[] array = fields;
	...
	internal void CallOnChange(FieldInfo field, object? oldValue, object? newValue)
	{
		...
		{
			MelonLogger.Error("Exception in OnChange handler\n" + ex.ToString());
		}
	}
	...
	private Visibility GetFieldVisibility(FieldInfo field)
	{
		...
		if (fieldVisibilities.TryGetValue(field, out Visibility value))
		{
			return value;
		}
